              ...
              itemBuilder: (context, index) {
                ...
                return Semantics(
                  ...
                  child: Card(
                    ...
                    child: ListTile(
                      title: Text('Game on ${game.startTime.toLocal().toString().split('.').first}'),
                      subtitle: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Players: ${game.players.map((p) => p.name).join(', ')}'),
                          Text('Frames: ${game.currentFrame - 1} | Winner: ${game.players.reduce((a, b) => a.score > b.score ? a : b).name}'),
                        ],
                      ),
                      ...
                      onTap: () {
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (context) => GameResultsScreen(completedGame: game),
                          ),
                        );
                      },
                    ),
